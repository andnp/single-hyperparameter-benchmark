...
## Unique to this repo
There are a few new scripts in this repo for managing and organizing experiments that I am playing with.
The first script/concept is **archive** and the second is **report**.

### Archives
An **archived** experiment is a previous version of an experiment that shares the same experiment name, but contains some out-dated data.
This often happens when a bug is found in the code or some non-observable experimental details change (different way of initializing the neural network, run for a different number of steps, etc.).
To create an **archive** run:
```bash
python scripts/archive.py <experiment name>
```
where `experiment name` is the name of the folder under `experiments/`.
For instance, imagine we had this directory structure:
```
experiments/
...
then running:
```bash
python scripts/archive.py MountainCar
```
would generate this archive:
```
experiments/
experiments/archive/MountainCar/v1
experiments/MountainCar
experiments/Tests

results/
results/archive/MountainCar/v1
```
Notice that the experiment files are **copied** into the archive, while the results files are **moved** into their archive.
This allows running the experiment scripts again without the potential of accidentally including old results with the new version of the experiments scripts.
When including the plotting scripts within the experiment directory, this also allows the plotting code to be archived alongside the experiment files and the corresponding results.

### Reports
A **report** is a markdown file for a given experiment that shows the plots of all past results of an experiment, as well as the current results for that experiment.
Reports make it easy to compare past results with new results and ensure that we are not regressing as well adapt and change our experiments.
Imagine an experiment has the following structure:
```
experiments/
experiments/MountainCar/
experiments/MountainCar/QLearning.json
experiments/MountainCar/notes.txt
experiments/MountainCar/learning_curve.py
```
Running:
```bash
python scripts/report.md MountainCar
```
will generate a file `experiments/MountainCar/report.md` that contains (in newest -> oldest order) each of the versions of this experiment, the `notes.txt` contents for that version, and the plots generated by each `*.py` file contained within the experiment (including for all past versions).
